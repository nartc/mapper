...
import { Rocket } from './fixtures/models/space-x/rocket';
import { RocketDto } from './fixtures/models/space-x/rocket.dto';
import { rocketProfile } from './fixtures/profiles/rocket.profile';
import { getRocket } from './utils/get-rocket';

describe('SpaceX API', () => {
  const [mapper] = setupClasses('spacex', {
    source: new SnakeCaseNamingConvention(),
    destination: new CamelCaseNamingConvention(),
  });

  it('should map', () => {
    mapper.addProfile(rocketProfile);

    const rocket = getRocket();
    const dto = mapper.map(rocket, RocketDto, Rocket);
