...
Nx v12.8 adds support for TypeScript Compiler plugins via an option called `tsPlugins` (now `transformers`) in their `@nrwl/node:webpack` executor, which is what `@nx/nest` is using for building the application.
