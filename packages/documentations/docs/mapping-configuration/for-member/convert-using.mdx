...
If we have simple logic, we can use [TODO: Type Converter docs](.) to also map from one data type to another on the Mapping level instead of `convertUsing` on the property level.
