...
Previously, `createMap()` is a method available on the `Mapper` object (returned by `createMapper()`). Now, `createMap()` is a standalone function instead.
...
Same as `createMap()`, `addProfile()` is a method on the `Mapper` in previous versions. Now, `addProfile()` is a standalone function
