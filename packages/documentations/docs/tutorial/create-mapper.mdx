...
-   The rest of the tutorial will apply to `@automapper/classes` only. If you're using `@automapper/pojos`, please check [TODO: POJOs Strategy Documentations](.)
...
`createMapper()` accepts a `CreateMapperOptions` which allows to customize the `Mapper`. Read more at [TODO: createMapper() API](.)
...
Read more about [TODO: @AutoMap() documentations](.)

:::

:::tip

If you want to remove the verbosity with `@AutoMap()`, check out [TODO: Transformer Plugin documentations](.)
